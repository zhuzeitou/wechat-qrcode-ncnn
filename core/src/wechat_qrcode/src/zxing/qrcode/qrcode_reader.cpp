...
vector<Ref<Result>> QRCodeReader::decodeMore(Ref<BinaryBitmap> image, Ref<BitMatrix> imageBitMatrix,
    ...
    for (int tryTimes = 0; tryTimes < 1; tryTimes++) {
        ...
        for (int i = 0; i < possiblePatternCount; i++) {
            ...
            auto bottomLeft = patternInfo->getBottomLeft();
            auto topLeft = patternInfo->getTopLeft();
            auto topRight = patternInfo->getTopRight();
            ...
            for (int j = 0; j < possibleAlignmentCount; j++) {
                ...
                Ref<AlignmentPattern> alignmentPattern = detector->getAlignmentPattern(i, j);
                ...
                bool hasAlignment = decoderResult->getQRCodeVersion() >= 2;
                int pointCount = hasAlignment ? 4 : 3;
                ArrayRef<Ref<ResultPoint> > result_points(new Array<Ref<ResultPoint> >(pointCount));
                result_points[0].reset(bottomLeft);
                result_points[1].reset(topLeft);
                result_points[2].reset(topRight);
                if (hasAlignment) {
                    result_points[3].reset(alignmentPattern);
                }

                if (decoderResult->getOtherClassName() == "QRCodeDecoderMetaData") {
                    decoderResult->getOther()->applyMirroredCorrection(result_points);
                }
                Ref<Result> result(
                    new Result(decoderResult->getText(), decoderResult->getRawBytes(), result_points,
                ...
                if (needTryVariousDeimensions[j]) {
                    ...
                    for (size_t k = 1; k < possibleDimensions.size(); k++) {
                        ...
                        Ref<Result> result(new Result(
                            decoderResult->getText(), decoderResult->getRawBytes(), result_points,
                            decoderResult->getCharset(), decoderResult->getQRCodeVersion(),
                            decoderResult->getEcLevel(), decoderResult->getCharsetMode()));
